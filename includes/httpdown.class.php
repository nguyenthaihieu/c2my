...
class HttpDown
{
	...
	function printError()
	{
		echo lang('error_message').$this->m_error;
		echo "<br/>".lang('Specific_returnhead')."<br/>";
	...
	function IsText()
	{
		...
		{
			$this->m_error .= lang('content_urlredirection')."<br/>";
			return false;
		}
	}
	...
	function IsContentType($ctype)
	{
		...
		{
			$this->m_error .= lang('Wrong_type').$this->GetHead("content-type")."<br/>";
			return false;
		}
	}
	...
	function SaveToBin($savefilename)
	{
		...
		if(@feof($this->m_fp))
		{
			$this->m_error = lang('Connection_closed'); return false;
		}
	...
	function PrivateStartSession($requestType="GET")
	{
		...
		{
			$this->m_error .= lang('Error_open_host');
			return false;
		}
		$this->reTry++;
		...
		if(ereg("^3",$this->m_httphead["http-state"]))
		{
			...
			{
				$this->m_error = lang('not_recognize_answer');
			}
		}
	}
